...
export default function StudentDashboard() {
  ...
  return (
    <>
      <ProfileBanner
        title={user?.name || "User"}
        subtitle={[decoded?.email || user?.email || "Student", user?.courseName].filter(Boolean).join(" • ")}
        logoUrl={null}
        role="STUDENT"
      />
          ...
          >
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                {user?.courseName && (
                  <p className="mt-2 inline-flex items-center gap-2 rounded-lg bg-blue-500/20 px-3 py-1.5 text-sm font-medium text-blue-300">
